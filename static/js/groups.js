$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'success') {
                    $('.parent-div').empty()
                    $.each(response.datas, function (index, group) {
                        add_group(group)
                    })
                    // verify list lengh
            ...
            error: function (xhr, status, error) {
                message('error', 'Erro!', xhr.responseText, false, null, '.ModalGroup').modal('show');
            }
        });
    }
    ...
    socket.on('group_delele', function (datas) {
        alert(datas.id)
    ...
    $('#btnSaveGroup').click(function (event) {
        ...
        if (!id) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'success') {
                        message('success', 'Sucesso', 'Informações do grupo atualizadas com sucesso.', false, null, '.ModalGroup').modal('show')
                        // clean form for new save
                        cleanFormGroup()
                    } else {
                        message('error', 'Erro!', response.msg, false, null, '.ModalGroup').modal('show');
                    }
                },
                error: function (xhr, status, error) {
                    message('error', 'Erro!', xhr.responseText, false, null, '.ModalGroup').modal('show');
    ...
    $('.parent-div').on('click', '.action_delete', function (event) {
        ...
        let view_item = $(this).closest('.view_item')
        let id = view_item.attr('id')
        $('#ModalMessage').attr('action_value', id)

        message('info', 'Eliminar grupo', `Tem a certeza que deseja eliminar o grupo <strong>${view_item.find('.label').text()}</strong>?`,
            false, null, null, 'Não', 'Sim', 'delete_group').modal('show')
    })
...
function add_group(group, prepend = false) {
    var new_div =
        `<div class="container-fluid">
            <div class="row">
                <div class="view_item col new d-flex align-items-center overflow-hidden m-0 p-0" id="${group.id}">
                    ...
                    <div class="info">
                        <div>
                            ...
                            <div>
                                <div class="text-truncate fw-bold label">
                                    ${group.label}
                                </div>
                            </div>

                            <div class="block">
                                <div class="text-truncate fw-bold name">
                                    ${group.name}
                                </div>
                            </div>
                            ...
                            <div class="block2">
                                <div class="text-truncate description">
                                    Os utilizadores desse grupo têm ${group.description}
                                </div>
                            </div>
                    ...
                    <div class="actions container px-4">
                        ...
                        <div class="rounded-circle action_delete" title="Eliminar" group_id="${group.id}">
                        ...
                        <div class="rounded-circle action_edit" title="Editar" group_id="${group.id}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                                class="bi bi-textarea-resizebi d-block" viewBox="0 0 16 16">
                                <path
    ...
    if (prepend)
        $('.list-group').prepend(new_div)
    else
        $('.list-group').append(new_div)
}